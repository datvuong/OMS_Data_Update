QueryTableSummaryData <- function(data, tableName, dbName, timeField,
  ...
  output <- tryCatch({
    ...
    query <- 
      paste0("SELECT DATE_FORMAT(", timeField, ", '%Y%m') month, COUNT(*) records, max(", timeField, ") latest_timestamp
