BuildPackageBasedDataSummary <- function(ventureShort, xMonth = 6, startDate = Sys.Date()) {
  suppressMessages({
    require(dplyr)
    require(tools)
    require(magrittr)
  ...
  functionName <- "BuildPackageBasedDataSummary"
  tableName <- "packageBasedData"
  ...
  output <- tryCatch({
    dataPath <- file.path("3_Script/3_RData", ventureShort, "packageDataBased")
     ...
     flog.info(paste("Function", functionName, "Month Process", iMonth, sep = "-"), name = reportName)
      tryCatch({
        if (file.exists(file.path(dataPath, iMonth))) {
          ...
          rm(list = c("basedDataMonth", "statusTimestampDataMonth"))
        }
      }, error = function(err){
