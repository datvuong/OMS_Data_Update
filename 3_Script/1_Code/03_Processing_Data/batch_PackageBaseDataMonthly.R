...
source("3_Script/1_Code/03_Processing_Data/BuildPackageBasedData.R")
...
flog.appender(appender.tee(file.path("3_Script/2_Log",
              ...
              name = reportName)
layout <- layout.format(paste0(timeReport, '|', reportName, '|[~l]|[~t]|[PackageBasedData]|~m'))
flog.layout(layout, name=reportName)

tryCatch({
  
  savedFolder <- file.path("3_Script/3_RData", ventureShort, "packageDataBased")
  if (!dir.exists(savedFolder)) {
    dir.create(savedFolder)
  }
  
  month <- format(as.POSIXct(startDate, format = "%Y-%m-%d"), "%Y%m")
  monthDataName <- paste0("packageBasedData_", month)
  
  eval(substitute(
    {
      data <- BuildPackageBasedData(venture = ventureShort, startDate = startDate, monthsGoBack = monthsGoBack)
    },
