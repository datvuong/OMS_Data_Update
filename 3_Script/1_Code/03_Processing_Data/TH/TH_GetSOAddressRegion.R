GetSOAddressRegion <- function(SOAddressData, addressRegionData) {
  ...
  output <- tryCatch({
    ...
    addressRegionData_mapped <- addressRegionData_rev %>%
      ...
      select(level_7_id_customer_address_region = id_customer_address_region,
             level_7_code = code,
             level_7_name = name,
             level_7_customer_address_region_type = customer_address_region_type,
             level_7_fk_customer_address_region = fk_customer_address_region)
    
    addressRegionData_mapped <- left_join(addressRegionData_mapped,
                                          addressRegionData_rev %>%
                                            select(level_6_id_customer_address_region = id_customer_address_region,
                                                   level_6_code = code,
                                                   level_6_name = name,
                                          ...
                                          by = c("level_7_fk_customer_address_region" = "level_6_id_customer_address_region"))
    ...
    SOAddressRegion <- left_join(SOAddressData, addressRegionData_mapped,
                                 by = c("fk_customer_address_region" = "level_7_id_customer_address_region"))
